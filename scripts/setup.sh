...
for REQUIRED_TOOL in ruby git node; do
  if ! command -v "$REQUIRED_TOOL" >/dev/null 2>&1; then
    printf 'setup: %s is required. Install Git 2.28+, Ruby 2.6+, and Node.js 22.13+, then rerun this command.\n' "$REQUIRED_TOOL" >&2
    exit 1
  fi
done
ruby -e '
  requirements = [["Ruby", RUBY_VERSION, [2, 6]], ["Git", ARGV[0], [2, 28]], ["Node.js", ARGV[1], [22, 13]]]
