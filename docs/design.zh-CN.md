...
MyContext 的核心是让个人资料成为 AI 可以按需阅读、引用和提出修改的文件。
黑客松演示可以围绕这个过程展开：问一个涉及项目和人物的问题，找到几份有来源的资料，
查看它们的联系，再提出一份由用户审阅的更新。不要把尚未实现的自动执行能力写成产品成果。

## 开源隔离

公开仓库包含代码、通用 Skills、零记录的空白模板，以及明确区分虚构生活故事与真实公开参考资料的演示。
...
这版不增加关系字段或推理引擎，也不重做前端。先验证检索和入门体验，再用真实使用问题
确定图谱升级范围。

## 可以演示的故事

1. 一名新用户克隆代码，运行演示，无需导入个人资料或配置模型密钥。
2. 用自己的本地 AI 询问里斯本旅行现在考虑什么日期、为什么改期，Librarian 找到当前计划和旧记录。
3. 打开 Graph，查看相关人物、项目和记录；说明连线是资料链接。
4. 用户提供一个新事件，AI 准备有来源和差异的更新提案。
5. 用户审阅提案；明确说明应用、提交和同步目前依赖助手执行的文档化流程，
   不是前端已经实现的一键自动执行功能。

新版演示有 95 条记录：70 条虚构的个人生活资料，25 条有官方来源的公开参考资料。
朋友、家人和个人经历都是虚构的；勒古恩、萨克斯、真实书籍、课程与地点标记为公开参考，
不会被写成虚构用户的真实人脉。`npm run setup:empty` 提供零条知识记录的起点。
