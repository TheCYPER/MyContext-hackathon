...
需要 Git 2.28+、Node.js 20+、Ruby 2.6+ 和 POSIX shell，支持 macOS、Linux 和 WSL。
应用没有需要另装的 npm 依赖或 Ruby gems，也不要求模型密钥或数据库。
...
打开 **http://127.0.0.1:4318**。端口被占用时，使用
...
相关经历、人物、工作日记和想法。使用 **Grow connections +** 逐层展开，
或通过 **Find a path** 查看两个记录之间的连接路径。

带类型的关系说明“参与”“属于”“关于”“支持”或“替代”等具体含义，并保留来源、
证据类别和审阅状态。普通链接与 `context:<id>` 来源引用只表示记录间的连接；
**Possible connections** 根据共同标签或邻居给出可供审阅的建议。
点击 **Prepare link for review** 可以复制修改建议，交给助手按资料库规则审阅。
