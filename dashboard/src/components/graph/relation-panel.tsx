...
export function RelationPanel({ relations, selectedRelationId, focusId, entities }: { relations: LegacyRelation[]; selectedRelationId: string | null; focusId: string; entities: Entity[] }) {
  ...
  const firstDeclaration = declarations[0] || { from: selected.from, to: selected.to };
  const mutual = declarations.some((item) => item.from === firstDeclaration.to && item.to === firstDeclaration.from);
  ...
  return <Card><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><div><Badge variant="secondary">Generic legacy link</Badge><h3 className="mt-3 font-semibold">{heading}</h3></div><ShieldAlert className="size-5 text-amber-600" /></div><dl className="mt-4 grid gap-2 text-sm"><div className="flex justify-between gap-3"><dt className="text-muted-foreground">Evidence</dt><dd>Reason not structured</dd></div><div className="flex justify-between gap-3"><dt className="text-muted-foreground">Review</dt><dd>{selected.review || "not represented"}</dd></div><div className="flex justify-between gap-3"><dt className="text-muted-foreground">Provenance</dt><dd>{selected.projectedProvenance || selected.provenance || "frontmatter.links"}</dd></div></dl><div className="mt-4 border-t pt-4"><h4 className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Recorded declarations</h4>{declarations.length ? declarations.map((item, index) => <p key={`${item.from}-${item.to}-${index}`} className="mt-2 text-xs">{byId.get(item.from)?.title || item.from} → {byId.get(item.to)?.title || item.to}{item.sourcePath ? ` · ${item.sourcePath}` : ""}</p>) : <p className="mt-2 text-xs">{byId.get(selected.from)?.title || selected.from} → {byId.get(selected.to)?.title || selected.to}</p>}</div></CardContent></Card>;
}
