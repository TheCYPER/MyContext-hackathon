...
export function GlobalGraph({
  ...
  const positions = layout.positions as Map<
    string,
    { x: number; y: number; width: number; height: number }
  >;
  return (
    <details className="mt-5 border bg-card">
        ...
        >
          {ATLAS_LANES.map(
            (lane: {
              ...
              <g key={lane.type}>
                <text x={lane.x} y={30} className="graph-lane-title">
                  {lane.label}
                </text>
                <line
                  x1={lane.x}
          ...
          {relations.map((edge) => {
            ...
            return (
              <line
                ...
                x2={to.x + to.width / 2}
                y2={to.y + to.height / 2}
                className="graph-edge graph-edge-global"
              />
            );
          })}
            ...
            return (
              ...
              >
                <rect
                  ...
                  rx={0}
                />
                <text x={box.x + 12} y={box.y + 23}>
                  <tspan className="graph-node-title">
                    {node.title.length > 26
                      ? `${node.title.slice(0, 24)}…`
                      : node.title}
                  </tspan>
                  <tspan x={box.x + 12} dy={17} className="graph-node-meta">
                    {node.type}
                  </tspan>
                </text>
              </g>
