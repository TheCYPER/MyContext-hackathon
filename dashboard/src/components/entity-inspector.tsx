      ...
      <DialogContent className="grid max-h-[88vh] grid-rows-[auto_minmax(0,1fr)_auto] p-0">
        <DialogHeader className="border-b px-6 pb-5 pt-6">
          <div className="flex flex-wrap items-center gap-2 pr-8">
            <Badge variant="secondary">{entity?.type || "record"}</Badge>
            <Badge variant="outline">{entity?.privacy || "loading"}</Badge>
          </div>
          <DialogTitle className="pr-8 text-xl">
            {entity?.title || "Loading record…"}
          </DialogTitle>
          <DialogDescription>
            Canonical record projected from committed context.
          </DialogDescription>
        </DialogHeader>
        <ScrollArea className="min-h-0 px-6 py-5">
          ...
          {error && (
            <div
              role="alert"
              className="border-l-4 border-signal bg-notice p-4 text-sm text-foreground"
            >
              {error}
            </div>
          )}
          {detail && (
            <div className="space-y-6">
              <div className="flex flex-wrap gap-2">
                {detail.entity.tags.map((tag) => (
                  <Badge key={tag} variant="secondary">
                    {tag}
                  </Badge>
                ))}
              </div>
              <dl className="grid gap-3 bg-muted/55 p-4 text-sm sm:grid-cols-3">
                <div>
                  <dt className="text-xs text-muted-foreground">Status</dt>
                  <dd className="mt-1 font-medium">{detail.entity.status}</dd>
                </div>
                <div>
                  <dt className="text-xs text-muted-foreground">Role</dt>
                  <dd className="mt-1 font-medium">
                    {detail.entity.role || "Not recorded"}
                  </dd>
                </div>
                <div>
                  <dt className="text-xs text-muted-foreground">Updated</dt>
                  <dd className="mt-1 font-medium">
                    {detail.entity.updated || "Not recorded"}
                  </dd>
                </div>
              </dl>
              {detail.entity.sections?.length ? (
                <div className="space-y-5">
                  {detail.entity.sections.map((section, index) => (
                ...
                <pre className="whitespace-pre-wrap break-words font-sans text-sm leading-7 text-foreground">
                  {detail.entity.body ||
                    ...
                    "No body text is available."}
                </pre>
              )}
              <section>
                <h3 className="text-sm font-semibold">Sources</h3>
                <ul className="mt-2 grid gap-1 text-sm text-muted-foreground">
                  {detail.entity.sources.length ? (
                    detail.entity.sources.map((source) => (
                      <li key={source}>{source}</li>
                    ))
                  ) : (
                    <li>No source locator recorded.</li>
                  )}
                </ul>
              </section>
              <section>
                <div className="flex items-center justify-between gap-3">
                  ...
                  {graphVisible && (
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => onFocusGraph(detail.entity.id)}
                    >
                      <Network />
                      Focus in graph
                    </Button>
        ...
        <footer className="flex flex-wrap items-center justify-between gap-2 border-t bg-muted/35 px-6 py-3 text-xs text-muted-foreground">
          <span className="inline-flex items-center gap-1.5">
            <FileText className="size-3.5" />
            {entity?.path || "Canonical path unavailable"}
          </span>
          <span className="inline-flex items-center gap-1.5">
            <GitCommitHorizontal className="size-3.5" />
            Read from tracked Git HEAD ·{" "}
            {(detail?.revision || revision).slice(0, 8)}
          </span>
        </footer>
      </DialogContent>
    </Dialog>
