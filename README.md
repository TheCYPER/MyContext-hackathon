...
Requires **Git 2.28+, Node.js 20+, Ruby 2.6+, and a POSIX shell** on macOS,
Linux, or Windows through WSL. No `npm install` or Ruby gems are needed.
...
seconds while the page is visible. Commit approved note changes to see them; use
**Refresh** for an immediate check. It is a read-only view; you work with your
...
Check Git 2.28+, Node.js 20+, Ruby 2.6+, and a POSIX shell. Report missing
prerequisites rather than installing system packages without permission.

Run npm run setup and npm test. Install project-scoped skills into
...
In the dashboard, **Grow connections +** reveals more connected records.
**Possible connections** suggests links based on shared tags or neighbors and
provides proposals to copy for owner review. Suggestions are not saved facts.
See [graph controls](dashboard/README.md#automatic-growth-and-link-review).
